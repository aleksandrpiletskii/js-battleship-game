...
function getValidity(allBoardBlocks, isHorizontal, startIndex, ship) {
    let validStart = isHorizontal ? startIndex <= width * width - ship.lenght ? startIndex :
    width * width - ship.lenght : startIndex <= width * width - ship.lenght ? startIndex : startIndex - ship.lenght * width + width
...
function startGame() {
    if (playerTurn === undefined) {
        if (optionContainer.children.length != 0) {
            infoDisplay.textContent = "Please place all your piecis first"
        } else {
...
function checkScore(user, userHits, userSunkShips) {
    ...
    if (playerSunkShips.length === 5) {
        infoDisplay.textContent = "You sunk all the somputers ships. YOU WON!"
        gameOver = true
    }
